...
export default function Dashboard() {
  ...
  React.useEffect(() => {
    dispatch(Request());
    axios.get(`http://localhost:${process.env.REACT_APP_JSON_SERVER_PORT}/jobs?_page=${page}&_limit=4`, {
      headers: {
          ...
          {state.isLoading
            ...
            : <>
              {state.data.map((job, idx) => {
                return (
                  <JobCard key={idx} title={job.title} company={job.company} location={job.location} description={job.description} requirements={job.requirements} responsibilities={job.responsibilities} />
                )
              })
              }
            </>
